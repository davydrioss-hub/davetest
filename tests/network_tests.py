...
try:
    ...
    wait(lambda:'Все места' in clients[6].state()['message'],'Fifth vehicle occupant rejected')
    ...
    wait(lambda:world(restored2)['economy']['shift']['day']==2 and not world(restored2)['items'] and len(world(restored2)['orders'])==30 and not player(restored2)['inventory'],'Next night reliably deletes prior cargo and preserves world')
    ...
    # Expansion commands travel through real RPC and all rewards remain on the host.
    empty_client.action('buy_vehicle','courier_02')
    wait(lambda:'автосалоне' in empty_client.state()['message'],'Remote fleet purchase rejected over RPC')
    for index,pos in enumerate([[-190,0,-120],[-174,0,-44],[-120,0,32]]):
        empty_host.command(teleport={empty_client.id:pos})
        wait(lambda pos=pos:abs(player(empty_client)['pos'][0]-pos[0])<.1,'Explorer reaches landmark '+str(index))
        empty_client.action('collect_parts',f'parts_{index:02}')
        wait(lambda index=index:world(empty_client)['economy']['company']['parts']==(index+1)*2,'Collected parts replicate '+str(index))
        empty_client.action('collect_parts',f'parts_{index:02}')
        wait(lambda:'доступному ящику' in empty_client.state()['message'],'Collected landmark cannot pay twice '+str(index))
    for index,pos in enumerate([[-106,0,-57],[106,0,-94],[48,0,98]]):
        empty_host.command(teleport={empty_client.id:pos})
        wait(lambda pos=pos:abs(player(empty_client)['pos'][0]-pos[0])<.1,'Mechanic reaches roadside call '+str(index))
        aid=f'service_{index:02}';empty_client.action('service_start',aid)
        wait(lambda aid=aid:world(empty_client)['activities'][aid]['status']=='working','Repair reservation replicates '+str(index))
        empty_client.action('service_finish',aid)
        wait(lambda:'окончания ремонта' in empty_client.state()['message'],'Early service reward rejected '+str(index))
        wait(lambda aid=aid:world(empty_host)['time']>=world(empty_host)['activities'][aid]['finish_at'],'Authoritative repair timer elapses '+str(index))
        empty_client.action('service_finish',aid)
        wait(lambda index=index:world(empty_client)['economy']['company']['repairs']==index+1,'Validated repair pays and replicates '+str(index))
    empty_client.action('accept_order','d001_01')
    wait(lambda:world(empty_client)['orders']['d001_01']['status']=='active','Expansion courier accepts contract')
    item=world(empty_host)['orders']['d001_01']['item'];pos=world(empty_host)['items'][item]['pos']
    empty_host.command(teleport={empty_client.id:pos})
    wait(lambda:abs(player(empty_client)['pos'][0]-pos[0])<.1,'Expansion courier reaches cargo')
    empty_client.action('pickup',item)
    wait(lambda:item in player(empty_client)['inventory'],'Expansion cargo pickup replicates')
    empty_host.command(teleport={empty_client.id:[-16,0,-8]})
    wait(lambda:abs(player(empty_client)['pos'][0]+16)<.1,'Expansion courier reaches destination')
    empty_client.action('complete_order','d001_01')
    wait(lambda:world(empty_client)['economy']['company']['balance']==2260,'Road calls and delivery have exact conserved budget')
    empty_host.command(teleport={empty_client.id:[-43,0,16]})
    wait(lambda:abs(player(empty_client)['pos'][2]-16)<.1,'Campaign terminal reached')
    empty_client.action('claim_chapter','0')
    wait(lambda:world(empty_client)['economy']['company']['chapter']==1 and world(empty_host)['economy']['company']['balance']==2560,'Chapter reward validated and replicated')
    empty_host.command(teleport={empty_client.id:[-141,0,56]})
    wait(lambda:abs(player(empty_client)['pos'][0]+141)<.1,'Buyer reaches western dealership')
    empty_client.action('buy_vehicle','courier_02')
    wait(lambda:world(empty_client)['vehicles']['courier_02']['owned'] and world(empty_client)['economy']['company']['balance']==360,'Client purchase adds shared second car with exact debit')
    city_late=Bot('expanded-late-join',password='')
    wait(lambda:connected(city_late),'Late join into expanded city')
    wait(lambda:world(city_late)['vehicles']['courier_02']['owned'] and world(city_late)['economy']['company']['chapter']==1 and world(city_late)['activities']['parts_00']['status']=='completed','Late snapshot includes fleet, campaign and exploration')
    empty_host.command(teleport={empty_client.id:[-149,0,58]})
    ...
    save=json.loads(json.loads((empty_host.path/'Saves/NetworkTest/world.sav').read_text())['payload'])
    assert save['vehicles']['courier_02']['owned'] and save['economy']['company']['chapter']==1
    CHECKS.append('Expanded host save persists purchased car and campaign');print('PASS Expanded host save persists purchased car and campaign',flush=True)
    ...
    empty_client.stop();city_late.stop()
    for bot in BOTS:
